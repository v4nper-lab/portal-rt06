...
if not df.empty:
    ...
    if menu == "Beranda / Dashboard":
        ...
        if not col_status_rumah:
            col_status_rumah = next((c for c in df.columns if "RUMAH" in c), None)
            
        col_status_domisili = next((c for c in df.columns if "DOMISILI" in c), None)
        if not col_status_domisili:
            col_status_domisili = next((c for c in df.columns if "STATUS" in c and "DOMISILI" in c), None)
    ...
    st.info("Pastikan file Excel data warga RT 06 sudah tersedia.")import streamlit as st
import pandas as pd
import plotly.express as px
import os
import io
