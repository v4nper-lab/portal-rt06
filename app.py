...
# Header Utama Portal RT 06 (Logo & Judul Diperbesar 3x, Warna Putih Logo Transparan)
...
if df.empty:
    ...
    if menu == "Beranda / Dashboard":
        ...
        with col_m1:
            if st.button("📋 Data Seluruh Warga", use_container_width=True, key="btn_main_warga_v22"):
                st.session_state.selected_menu = "📋 Data Seluruh Warga"
                st.rerun()
            if st.button("🗂️ Cetak Kartu Keluarga (KK)", use_container_width=True, key="btn_main_kk_v22"):
                st.session_state.selected_menu = "🗂️ Cetak Kartu Keluarga (KK)"
                st.rerun()
            if st.button("📊 Rekapitulasi Administrasi RW", use_container_width=True, key="btn_main_rw_v22"):
                st.session_state.selected_menu = "📊 Rekapitulasi Administrasi RW"
                st.rerun()
        with col_m2:
            if st.button("📈 Grafik Demografi", use_container_width=True, key="btn_main_grafik_v22"):
                st.session_state.selected_menu = "📈 Grafik Demografi"
                st.rerun()
            if st.button("💰 Laporan Kas RT & Sosial", use_container_width=True, key="btn_main_kas_v22"):
                st.session_state.selected_menu = "💰 Laporan Kas RT & Sosial (Perelek R6 Sauyunan)"
                st.rerun()
            if st.button("🖨️ Cetak Laporan Rekap PDF", use_container_width=True, key="btn_main_pdf_v22"):
        ...
        if st.button("⬅️ Kembali ke Beranda", key="back_from_warga_v22"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        ...
        keyword_cari = st.text_input("🔍 Ketik Nama Warga yang Ingin Dicari untuk Diperbaiki:", key="keyword_cari_warga_edit_v11")
        ...
        if keyword_cari.strip():
            ...
            if not df_hasil_cari.empty:
                ...
                pilih_warga_edit = st.selectbox("Pilih Data Warga yang Ditemukan:", pilihan_edit_str, key="select_warga_to_edit_v11")
        ...
        with st.form("form_hapus_warga_stabil_bawah_v22", clear_on_submit=False):
            list_warga_pilih = [f"Baris {i+1} | Rumah: {row.get(col_rumah, '-')} | KK: {row.get(col_kk, '-')} | Nama: {row.get(col_nama, '-')}" for i, row in df.iterrows()]
            target_hapus_str = st.selectbox("Pilih Warga yang Ingin Dihapus:", ["(Pilih warga...)"] + list_warga_pilih, key="sel_hapus_bawah_v22")
        ...
        with st.form("form_input_warga_lengkap_v17", clear_on_submit=False):
            col_f1, col_f2 = st.columns(2)
            with col_f1:
                in_no_rumah = st.selectbox("No. Rumah", sorted(list(set(daftar_blok_lengkap))), key="in_no_rumah_full_v17")
                in_nama_kk = st.text_input("NAMA KEPALA KELUARGA", key="in_nama_kk_full_v17")
                in_nama_anggota = st.text_input("Nama Lengkap Anggota Keluarga", key="in_nama_anggota_full_v17")
                in_jk = st.selectbox("Jenis Kelamin", ["L", "P"], key="in_jk_full_v17")
                in_hub = st.selectbox("Hubungan Keluarga", ["Kepala Keluarga", "Istri", "Anak Kandung", "Famili Lain", "Mertua"], key="in_hub_full_v17")
                in_tmplhr = st.text_input("Tempat Lahir", key="in_tmplhr_full_v17")
                
                st.markdown("📅 **Tanggal Lahir (Tgl, Bulan, Tahun):**")
                c_t1, c_t2, c_t3 = st.columns(3)
                with c_t1: in_tgl = st.number_input("Tgl", min_value=1, max_value=31, value=1, key="in_tgl_v17")
                with c_t2: in_bln = st.number_input("Bln", min_value=1, max_value=12, value=1, key="in_bln_v17")
                with c_t3: in_thn = st.number_input("Thn", min_value=1900, max_value=2026, value=1995, key="in_thn_v17")
            ...
            with col_f2:
                in_status_nikah = st.selectbox("Status Perkawinan", ["Belum Kawin", "Kawin", "Cerai Hidup", "Cerai Mati"], key="in_status_nikah_full_v17")
                in_agama = st.selectbox("Agama", ["Islam", "Kristen", "Katolik", "Hindu", "Buddha", "Konghucu"], key="in_agama_full_v17")
                in_goldarah = st.selectbox("Gol. Darah", ["A", "B", "AB", "O", "Tidak Tahu"], key="in_goldarah_full_v17")
                in_suku = st.selectbox("Etnis / Suku", ["Sunda", "Jawa", "Padang", "Batak", "Betawi", "Lainnya"], key="in_suku_full_v17")
                in_pend = st.selectbox("Pendidikan", ["Tamat SLTA/sederajat", "Tamat SLTP/sederajat", "Tamat SD/sederajat", "Diploma IV / Strata I", "Sedang SD/sedajerat", "Sedang SLTP/sederajat", "Belum / Tidak Sekolah"], key="in_pend_full_v17")
                in_pek = st.selectbox("Pekerjaan", ["Karyawan Swasta", "Wiraswasta", "Mengurus Rumah Tangga", "Belum Bekerja", "Pelajar", "PNS / TNI / Polri"], key="in_pek_full_v17")
                in_status_rumah = st.selectbox("Status Rumah", ["Milik / Tetap", "Sewa/Kontrak", "Kosong"], key="in_status_rumah_full_v17")
                in_status_domisili = st.selectbox("Status Domisili", ["Nanjung Mekar", "luar NM"], key="in_status_domisili_full_v17")
        ...
        # Ambil daftar unik berdasarkan Nomor Rumah atau Kepala Keluarga
        list_kk_ opsi = []
        for _, r_item in df_ffill.iterrows():
            ...
            if kk_val and kk_val.lower() not in ['', 'nan', 'none']:
                ...
                if display_str not in list_kk_ opsi:
                    list_kk_ opsi.append(display_str)

        pilihan_kk_str = st.selectbox("Pilih Rumah / Kepala Keluarga:", sorted(list_kk_ opsi), key="select_kk_main_v22")
        
        if pilihan_kk_str:
            # Ekstrak No. Rumah dan KK dari pilihan
            parts = pilihan_kk_str.split(" - KK: ")
            rmh_target = parts[0].replace("Rumah ", "").strip()
            kk_target = parts[1].strip()

            # Filter anggota keluarga berdasarkan No. Rumah yang sama atau KK yang sama
            if col_rumah:
            ...
            # FITUR CETAK PDF KARTU KELUARGA
            st.markdown("---")
        ...
        if st.button("⬅️ Kembali ke Beranda", key="back_from_grafik_v22"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        ...
        if col_jk:
            ...
            st.plotly_chart(fig_jk, use_container_width=True, key="chart_jk_pie_main_v9")
        ...
        if col_status:
            ...
            st.plotly_chart(fig_st, use_container_width=True, key="chart_status_bar_main_v9")
        ...
        if col_pend:
            ...
            st.plotly_chart(fig_pd, use_container_width=True, key="chart_pend_bar_main_v9")
        ...
        if col_pek:
            ...
            st.plotly_chart(fig_pk, use_container_width=True, key="chart_pek_bar_main_v9")
        ...
        if col_s_rumah:
            ...
            st.plotly_chart(fig_sr, use_container_width=True, key="chart_s_rumah_bar_main_v8")
        ...
        if col_dom:
            ...
            st.plotly_chart(fig_dm, use_container_width=True, key="chart_domisili_pie_main_v3")
        ...
        if col_usia:
            ...
            st.plotly_chart(fig_usia, use_container_width=True, key="chart_usia_pie_main_v9")

    elif menu == "📊 Rekapitulasi Administrasi RW":
        if st.button("⬅️ Kembali ke Beranda", key="back_from_rw_v22"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        ...
        if st.button("⬅️ Kembali ke Beranda", key="back_from_kas_v22"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        ...
        if st.button("⬅️ Kembali ke Beranda", key="back_from_pdf_v22"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
