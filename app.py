...
def load_data_rt06_stable():
    ...
    try:
        ...
        # Perbaikan ketat untuk kolom USIA agar tidak pernah menjadi angka ratusan abnormal
        ...
        if col_usia_candi:
            for idx_u, row_u in df.iterrows():
                ...
                try:
                    ...
                    # Jika usia tidak valid atau ratusan, hitung dari tanggal lahir jika ada
                    if col_tgl_lahir_candi:
                        ...
                        for p_str in tgl_str.split():
                            if p_str.isdigit() and len(p_str) == 4 and 1900 <= int(p_str) <= 2026:
                                ...
                                break
                        if not found_year:
                            usia_bersih = 30 # Default normal jika tahun tidak terdeteksi
                    else:
                        usia_bersih = 30
...
if not df.empty:
    ...
    if menu == "Beranda / Dashboard":
        ...
        with col_m1:
            if st.button("📋 Data Seluruh Warga", use_container_width=True, key="btn_m1"):
                ...
                st.rerun()
            if st.button("🗑️ Kelola & Hapus Warga", use_container_width=True, key="btn_m_hapus"):
                st.session_state.selected_menu = "🗑️ Kelola & Hapus Warga"
                st.rerun()
        ...
        with col_m2:
            if st.button("📈 Grafik Demografi", use_container_width=True, key="btn_m5"):
                ...
                st.rerun()
            if st.button("💰 Laporan Kas RT & Sosial", use_container_width=True, key="btn_m4"):
        ...
        if st.button("⬅️ Kembali ke Beranda", key="back_warga"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        st.subheader("📋 Data Keseluruhan Warga & Formulir Input Stabil")
        
        st.markdown("💡 **Panduan:** Kolom usia kini telah diperbaiki sehingga menampilkan angka normal warga. Gunakan formulir stabil di bawah untuk menambah warga baru tanpa loncat-loncat.")
        ...
        with st.form("form_input_warga_stabil_v8", clear_on_submit=False):
            col_f1, col_f2 = st.columns(2)
            tgl_lhr, bln_lhr, thn_lhr = 1, 1, 1995

            with col_f1:
                in_no_rumah = st.selectbox("No. Rumah", sorted(list(set(daftar_blok_lengkap))), key="in_no_rmh_stabil_v8")
                in_nama_kk = st.text_input("NAMA KEPALA KELUARGA", key="in_nama_kk_stabil_v8")
                in_nama_anggota = st.text_input("Nama Lengkap Anggota Keluarga", key="in_nama_anggota_stabil_v8")
                in_jk = st.selectbox("Jenis Kelamin", ["L", "P"], key="in_jk_stabil_v8")
                in_hub = st.selectbox("Hubungan Keluarga", ["Kepala Keluarga", "Istri", "Anak Kandung", "Famili Lain", "Mertua"], key="in_hub_stabil_v8")
                in_tmplhr = st.text_input("Tempat Lahir", key="in_tmplhr_stabil_v8")
                
                st.markdown("📅 **Tanggal Lahir:**")
                c_t1, c_t2, c_t3 = st.columns(3)
                with c_t1: tgl_lhr = st.number_input("Tgl", min_value=1, max_value=31, value=1, key="in_tgl_v8")
                with c_t2: bln_lhr = st.number_input("Bln", min_value=1, max_value=12, value=8, key="in_bln_v8")
                with c_t3: thn_lhr = st.number_input("Thn", min_value=1900, max_value=2026, value=1995, key="in_thn_v8")
            ...
            with col_f2:
                ...
                in_usia = st.number_input("Usia (Otomatis)", min_value=0, max_value=120, value=int(usia_otomatis), key="in_usia_stabil_v8")
                
                in_status_nikah = st.selectbox("Status Perkawinan", ["Belum Kawin", "Kawin", "Cerai Hidup", "Cerai Mati"], key="in_status_nikah_stabil_v8")
                in_agama = st.selectbox("Agama", ["Islam", "Kristen", "Katolik", "Hindu", "Buddha", "Konghucu"], key="in_agama_stabil_v8")
                in_goldarah = st.selectbox("Gol. Darah", ["A", "B", "AB", "O", "Tidak Tahu"], key="in_goldarah_stabil_v8")
                in_suku = st.selectbox("Etnis / Suku", ["Sunda", "Jawa", "Padang", "Batak", "Betawi", "Lainnya"], key="in_suku_stabil_v8")
                in_pend = st.selectbox("Pendidikan", ["Tamat SLTA/sederajat", "Tamat SLTP/sederajat", "Tamat SD/sederajat", "Diploma IV / Strata I", "Sedang SD/sedajerat", "Sedang SLTP/sederajat", "Belum / Tidak Sekolah"], key="in_pend_stabil_v8")
                in_pek = st.selectbox("Pekerjaan", ["Karyawan Swasta", "Wiraswasta", "Mengurus Rumah Tangga", "Belum Bekerja", "Pelajar", "PNS / TNI / Polri"], key="in_pek_stabil_v8")
                
                in_status_rumah = st.selectbox("Status Rumah", ["Milik / Tetap", "Sewa/Kontrak", "Kosong"], key="in_status_rumah_stabil_v8")
                in_status_domisili = st.selectbox("Status Domisili", ["Nanjung Mekar", "luar NM"], key="in_status_domisili_stabil_v8")
    ...
    elif menu == "🗑️ Kelola & Hapus Warga":
        if st.button("⬅️ Kembali ke Beranda", key="back_hapus_menu"):
            st.session_state.selected_menu = "Beranda / Dashboard"
            st.rerun()
        st.subheader("🗑️ Menu Khusus Penghapusan Data Warga (Pindah / Keluar)")
        st.markdown("💡 **Panduan:** Pilih warga yang sudah pindah melalui menu pilihan di bawah ini, lalu klik tombol merah untuk menghapusnya secara permanen dari file Excel server.")

        with st.form("form_menu_hapus_khusus_v2", clear_on_submit=False):
            list_warga_pilih = [f"Baris {i+1} | Rumah: {row.get(col_rumah, '-')} | KK: {row.get(col_kk, '-')} | Nama: {row.get(col_nama, '-')}" for i, row in df.iterrows()]
            target_hapus_str = st.selectbox("Pilih Warga yang Ingin Dihapus:", ["(Pilih warga...)"] + list_warga_pilih, key="select_warga_hapus_dropdown_v6")
